...
export default function KanbanBoard({ workspaceId: externalWorkspaceId }) {
  ...
  const createTask = (columnId) => {
    if (!data.columnOrder.length) {
      Alert.alert("No columns", "Create a column first");
      return;
    }
    setTaskModal({
      visible: true,
      columnId: columnId || data.columnOrder[0],
      task: null,
    });
  };
          ...
          try {
            ...
            setData((prev) => {
              ...
              Object.keys(newColumns).forEach((cid) => {
                newColumns[cid] = {
                  ...newColumns[cid],
                  taskIds: newColumns[cid].taskIds.filter(
                    (id) => id !== taskId,
                  ),
                };
              });
  ...
  const moveTask = (taskId, targetColumnId) => {
    ...
    setData((prev) => {
      ...
      Object.keys(newColumns).forEach((cid) => {
        newColumns[cid] = {
          ...newColumns[cid],
          taskIds: newColumns[cid].taskIds.filter((id) => id !== taskId),
        };
      });
    ...
    kanbanService
      .updateTask(taskId, { columnId: targetColumnId, order: newOrder })
      .catch((e) => {
        fetchData();
      });
  };
  ...
  const activeFilters =
    (filterPriority !== "all" ? 1 : 0) + (filterType !== "all" ? 1 : 0);
    ...
    <View className="flex-1" style={{ backgroundColor: palette.colors.page }}>
      ...
      <View className="px-4 pt-1 pb-2">
        <View className="flex-row items-center gap-2">
          <View
            className="flex-1 flex-row items-center rounded-2xl px-3 h-10"
          ...
          >
            <Ionicons
              name="funnel"
              size={16}
              color={activeFilters > 0 ? "#5eead4" : palette.textMutedColor}
            />
            {activeFilters > 0 && (
      ...
      >
            ...
            <KanbanColumnView
              ...
              onDeleteColumn={() => {
                if (column.taskIds.length > 0) {
                  Alert.alert(
                    "Delete Column",
                    `"${column.title}" has ${column.taskIds.length} tasks. Delete anyway?`,
                    [
                      { text: "Cancel", style: "cancel" },
                      {
                        text: "Delete",
        ...
        <TouchableOpacity
          ...
          style={{
            borderColor: palette.colors.border + "80",
            minHeight: 160,
            marginTop: 48,
          }}
        >
  ...
  return (
    <View
      ...
      style={{
        ...
        borderWidth: 1,
        borderColor: palette.colors.border + "60",
        maxHeight: "100%",
      }}
    >
      <View
      ...
      >
        ...
        <View className="flex-row gap-1">
          ...
          <View className="relative">
            <TouchableOpacity
              onPress={() => setShowColumnActions(!showColumnActions)}
              className="p-1.5 rounded-lg"
              style={{
                backgroundColor: isDark
                  ? "rgba(255,255,255,0.05)"
            ...
            {showColumnActions && (
              <View
                className="absolute right-0 top-8 rounded-2xl py-2"
                style={{
              ...
              >
                ...
                <View
                  ...
                  style={{
                    borderTopWidth: 1,
                    borderTopColor: palette.colors.border + "40",
                  }}
                />
                <TouchableOpacity
                  onPress={() => {
      ...
      <TouchableOpacity
        ...
        style={{
          backgroundColor: isDark
            ...
            : "rgba(0,0,0,0.03)",
          borderWidth: 1,
          borderColor: palette.colors.border + "30",
        }}
      >
        <Ionicons name="add" size={14} color={palette.textMutedColor} />
        <Text
...
function KanbanCardView({
  ...
  const [showActions, setShowActions] = useState(false);
  const doneChecklists = (task.checklists || []).filter(
    (c) => c.completed,
  ).length;
  ...
  return (
    <Pressable
      ...
      style={{
        ...
        borderWidth: 1,
        borderColor: palette.colors.border + "50",
      }}
    >
      <View
      ...
      <View className="flex-row items-start justify-between mb-2">
        <View className="flex-row items-center gap-1.5">
          <View
            ...
            style={{
              backgroundColor: (typeColors[task.type] || "#6366f1") + "20",
            }}
          >
            <Ionicons
              name={typeIcons[task.type] || "checkbox-outline"}
              size={12}
              color={typeColors[task.type] || "#6366f1"}
            />
          </View>
          <Text
        ...
        <View className="mb-2">
          <View className="flex-row items-center justify-between mb-1">
            <Text
              ...
              style={{ color: palette.textMutedColor + "99" }}
            >
              Checklist
            </Text>
            <Text
          ...
          >
            <View
              ...
              style={{
                width: `${(doneChecklists / totalChecklists) * 100}%`,
      ...
      <View
        className="flex-row items-center justify-between mt-1.5 pt-2"
        style={{
          borderTopWidth: 1,
          borderTopColor: palette.colors.border + "30",
        }}
      >
        <View className="flex-row items-center gap-2">
          <View
            ...
            style={{
              backgroundColor:
                (priorityColors[task.priority] || "#3b82f6") + "15",
            }}
          >
            <Text
              className="text-[8px] font-bold capitalize"
              style={{ color: priorityColors[task.priority] || "#3b82f6" }}
            >
              {task.priority}
            </Text>
          </View>
        ...
        {task.assignee && (
          ...
          >
            <Text className="text-[8px] font-bold text-teal-600">
              {task.assignee.displayName?.[0] || "U"}
            </Text>
          </View>
        )}
      </View>
      ...
      {showActions && (
        <View
          className="mt-2 pt-2 flex-row flex-wrap gap-1.5"
          style={{
            borderTopWidth: 1,
            borderTopColor: palette.colors.border + "30",
          }}
            ...
            colId !== task.columnId ? (
              ...
              >
                ...
                <Text
                  ...
                  numberOfLines={1}
                >
                  {columnIds.findIndex((id) => id === colId) !== -1
                    ? "Move"
                    : ""}
                </Text>
              </TouchableOpacity>
