...
export async function cloneColumn(id) {
  const { data } = await api.post(apiUrls.kanbanCloneColumn + `/${id}/clone`);
  return data;
}
...
export async function createChecklistItem(taskId, payload) {
  const { data } = await api.post(
    apiUrls.kanbanChecklists + `/${taskId}/checklists`,
    payload,
  );
  return data;
}
...
export async function updateChecklistItem(taskId, itemId, payload) {
  const { data } = await api.patch(
    apiUrls.kanbanChecklistItem + `/${taskId}/checklists/${itemId}`,
    payload,
  );
  return data;
}
