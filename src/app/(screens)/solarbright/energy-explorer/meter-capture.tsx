...
import type { CaptureMode } from '../../../../types/solarbright-energy';

const meterInputModes: Array<{
  id: CaptureMode;
  title: string;
  description: string;
  icon: string;
...
export default function MeterCaptureScreen() {
  ...
  function setPreviewAsset(asset: ImagePicker.ImagePickerAsset) {
  ...
  async function handleCaptureMode(mode: CaptureMode) {
    try {
      if (mode === 'camera') {
                ...
                {meterInputModes.map((mode) => (
                  ...
                  <View className="flex-row items-start">
                      <View className={`mr-3 h-11 w-11 items-center justify-center rounded-2xl ${palette.iconCard}`}>
                        <FontAwesome name={mode.icon as never} size={18} color={palette.iconColor} />
                      </View>
