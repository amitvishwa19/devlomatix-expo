  ...
  const renderCampaign = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        <View className="flex-row items-center justify-between">
          <View className="flex-1">
            ...
            <Text className={`mt-1 text-[12px] ${palette.textSoft}`}>
              {item.messageType || 'text'} {item.scheduledAt ? `| Scheduled: ${new Date(item.scheduledAt).toLocaleDateString()}` : ''}
            </Text>
          </View>
        </View>
        ...
        {total > 0 ? (
          <View className="mt-3">
            <View className="mb-1 h-2 rounded-full" style={{ backgroundColor: palette.colors.surfaceAlt }}>
        ...
        <View className="mt-3 flex-row gap-2">
          ...
          <TouchableOpacity onPress={() => handleClone(item)}
            ...
            <Ionicons name="copy-outline" size={16} color={palette.textColor} />
          </TouchableOpacity>
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.colors.page }}>
      <View className="flex-1 px-4 pt-5">
        <View className="mb-4 flex-row items-center justify-between">
          <View>
            <View className="mb-2 self-start rounded-full bg-sky-600 px-3 py-1.5">
              <Text className="text-[11px] font-bold uppercase tracking-[1px] text-white">CAMPAIGNS</Text>
            </View>
            <Text className="text-[28px] font-bold" style={{ color: palette.textColor }}>Broadcasts</Text>
            <Text className={`mt-1 text-[13px] ${palette.textSoft}`}>{campaigns.length} campaign{campaigns.length !== 1 ? 's' : ''}</Text>
          </View>
          <TouchableOpacity onPress={() => setShowCreate(true)}
            className="flex-row items-center gap-2 rounded-full bg-sky-600 px-5 py-3">
      ...
      <Modal visible={showCreate} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowCreate(false)}>
        <SafeAreaView className="flex-1" style={{ backgroundColor: palette.colors.page }}>
          <View className="flex-row items-center justify-between px-5 py-4" style={{ backgroundColor: palette.colors.surface }}>
            <Text className={`text-[20px] font-bold ${palette.text}`}>New Campaign</Text>
          ...
          <View className="flex-1 px-5 pt-6">
            <Text className={`mb-1 text-[13px] font-semibold ${palette.text}`}>Campaign Name *</Text>
            ...
            <View className="mb-4 flex-row gap-2">
              {['text', 'template'].map((t) => (
                <TouchableOpacity key={t} onPress={() => setForm({ ...form, messageType: t })}
                  className={`flex-1 items-center rounded-xl py-3 ${form.messageType === t ? 'bg-sky-600' : 'border'}`}
                  style={form.messageType !== t ? { borderColor: palette.colors.border } : {}}>
                  <Text className={`text-[13px] font-bold ${form.messageType === t ? 'text-white' : palette.text}`}>
                ...
                <View className="mb-4 flex-row flex-wrap gap-2">
                  {templates.map((t) => (
                    <TouchableOpacity key={t.id} onPress={() => setForm({ ...form, templateId: t.id })}
                      className={`rounded-xl px-4 py-2 ${form.templateId === t.id ? 'bg-sky-600' : 'border'}`}
                      style={form.templateId !== t.id ? { borderColor: palette.colors.border } : {}}>
                      <Text className={`text-[12px] font-semibold ${form.templateId === t.id ? 'text-white' : palette.text}`}>
                        {t.name}
                      </Text>
                    </TouchableOpacity>
                  ))}
            ...
            <Text className={`mb-1 text-[13px] font-semibold ${palette.text}`}>Message Text</Text>
      ...
      <Modal visible={!!showDetail} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowDetail(null)}>
        <SafeAreaView className="flex-1" style={{ backgroundColor: palette.colors.page }}>
          <View className="flex-row items-center justify-between px-5 py-4" style={{ backgroundColor: palette.colors.surface }}>
            <Text className={`text-[20px] font-bold flex-1 ${palette.text}`}>{showDetail?.name}</Text>
            <TouchableOpacity onPress={() => setShowDetail(null)} className="p-2">
              <Ionicons name="close" size={24} color={palette.textColor} />
            </TouchableOpacity>
          </View>
          ...
          <View className="flex-1 px-5 pt-6">
            {showDetail ? (
              <>
                <View className="mb-6 rounded-[20px] border p-5" style={{ backgroundColor: palette.colors.surface, borderColor: palette.colors.border }}>
                  <View className="flex-row items-center gap-2 mb-3">
                    <View className="rounded-full px-3 py-1" style={{ backgroundColor: (STATUS_BADGES[showDetail.status] || STATUS_BADGES.DRAFT).bg }}>
                      <Text style={{ color: (STATUS_BADGES[showDetail.status] || STATUS_BADGES.DRAFT).color, fontSize: 11, fontWeight: '800' }}>
                        {(STATUS_BADGES[showDetail.status] || STATUS_BADGES.DRAFT).label}
                      </Text>
                    </View>
                    <Text className={`text-[12px] ${palette.textSoft}`}>{showDetail.messageType || 'text'}</Text>
                  </View>

                  <View className="flex-row flex-wrap gap-4">
                    <View>
                      <Text className={`text-[11px] font-semibold ${palette.textMuted}`}>Total Recipients</Text>
                      <Text className={`text-[22px] font-bold ${palette.text}`}>{showDetail.total || showDetail._count?.recipients || 0}</Text>
                    </View>
                    <View>
                      <Text className={`text-[11px] font-semibold ${palette.textMuted}`}>Sent</Text>
                      <Text className={`text-[22px] font-bold ${palette.text}`}>{showDetail.sent || 0}</Text>
                    </View>
                    {showDetail.scheduledAt ? (
                      <View>
                        <Text className={`text-[11px] font-semibold ${palette.textMuted}`}>Scheduled</Text>
                        <Text className={`text-[14px] font-bold ${palette.text}`}>{new Date(showDetail.scheduledAt).toLocaleDateString()}</Text>
                      </View>
                    ) : null}
                  </View>
                </View>
                ...
                <View className="flex-row gap-3">
                  {(showDetail.status === 'DRAFT' || showDetail.status === 'PAUSED') ? (
                    <TouchableOpacity onPress={() => { handleToggle(showDetail); setShowDetail(null); }}
                      ...
                      <Text className="text-[15px] font-bold text-white">Start Campaign</Text>
                    </TouchableOpacity>
                  ) : null}
                  {showDetail.status === 'RUNNING' ? (
                    <TouchableOpacity onPress={() => { handleToggle(showDetail); setShowDetail(null); }}
                      className="flex-1 items-center rounded-xl bg-amber-600 py-4">
                      <Text className="text-[15px] font-bold text-white">Pause Campaign</Text>
                    </TouchableOpacity>
                  ) : null}
                  <TouchableOpacity onPress={() => { handleClone(showDetail); setShowDetail(null); }}
                    ...
                    <Text className={`text-[15px] font-bold ${palette.text}`}>Clone</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => { handleDelete(showDetail.id); setShowDetail(null); }}
                    className="items-center rounded-xl bg-red-600 px-6 py-4">
